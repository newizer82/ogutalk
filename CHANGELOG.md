...
### Added
- **활동 분류 2단계 세분화** — 그룹 4개(⚡생산 · 💬소통 · 🍿소비 · 🧭생활) + 세부 12개
  - SNS와 유튜브(동영상) 분리, 검색·게임·학습·AI·금융·촬영·구매/주문/예약 등 휴대폰 활용 패턴별 분류
  - 기준: 생산 = 만들거나 끝낸 행위(문서·메일·촬영·구매·예약), 소비 = 보고 읽기만 한 시간(검색 포함)
- 리포트에 그룹 분포 + 세부(앱 사용 기준) 분포 표시
- 자동 기록이 안드로이드 OS 앱 분류를 폴백으로 사용 → 미분류 앱 감소
- 성장 측정 이벤트 6종 (알람 노출·체크인·자동 기록 켜기·권한 허용·공유·로그인)

### Changed
- 알람 팝업 수동 선택지를 그룹 4개로 축소, 선택 없이 닫기 가능 (폰을 안 쓴 시간은 기록하지 않음)
- 휴식·식사 / 목표 할일 항목 제거 (옛 기록은 생활 / 생산으로 계속 표시)
- 공유 문구 개편 (문제 제시형)
- 주간 리포트 이야기 문구를 새 그룹 기준으로 변경 (서버 함수 별도 배포 필요)

### Fixed
- 홈 런처·공유 시트·시스템 화면이 "가장 많이 쓴 앱"으로 잡히던 문제
- 노션 패키지명 오류로 분류되지 않던 문제
- 옛 4분류로 저장된 앱 수정값이 새 분류 기준을 덮어쓰던 문제
...
### Added
- **자동 활동 기록** — 앱 사용시간(UsageStats)으로 30분마다 활동을 자동 기록 (기본 OFF, 사용자가 설정에서 켬)
